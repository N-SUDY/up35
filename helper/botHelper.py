...
async def search_user_in_community(bot, update):
    try:
        await bot.get_chat_member('@SL_BOTS_TM', update.chat.id)
        await bot.get_chat_member('@ImDark_Empire', update.chat.id)
    except UserNotParticipant:
        await update.reply_text(BotMessage.not_joined_community, parse_mode = 'html',reply_markup=InlineKeyboardMarkup([
        [InlineKeyboardButton('Join our Channel.',url = 'https://t.me/SLBotOfficial')],
        [InlineKeyboardButton('Join our Group.',url = 'https://t.me/trtechguide')]
        ]))
        return
